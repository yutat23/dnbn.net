...
具象型 `TcpServer` は `IAsyncDisposable` も実装します。`netstandard2.0` では `OnMessageReceivedAsync` にインターフェイスの既定実装がないため、このターゲットで独自の `ITcpServer` を実装する場合はイベントの定義が必要です。
...
| `ConnectionInfo` | 接続状態と統計（`IsReconnecting` と `KeepAliveTimeoutCount` を含む） |
...
具象型 `TcpClient` は `IAsyncDisposable` も実装します。`netstandard2.0` では `OnMessageTrace` にインターフェイスの既定実装がないため、このターゲットで独自の `ITcpClient` を実装する場合はイベントの定義が必要です。
...
コンストラクタに渡すか、DI に `IMessageFilter` 実装を登録します。`TcpMessengerFactory` は `IEnumerable<IMessageFilter>` として受け取り、生成するすべてのサーバーとクライアントに適用します。
