...
| 3 | 障害と自動再接続 | `ConnectionRetryPolicy`（無限リトライ）、`ConnectionInfo.IsReconnecting`、`InterruptReconnectDelay`、`WaitForConnectionAsync` |
...
| 7 | 運用監視 | `IMessageFilter`（チェックサム付与/検証）、`ConnectionInfo`統計、メッセージ履歴・送信を含む Web UI |
